...
export default function AISolutionsPage({ params }: { params: { locale: string } }) {
      ...
      <main>
        <section className="relative pt-32 pb-20 section-dark overflow-hidden">
          <div className="absolute inset-0">
            ...
            <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-96 h-96 rounded-full opacity-15 blur-3xl"
              style={{ background: '#6CACE4' }} />
          </div>
          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <span className="badge-dark mb-6">{a.badge}</span>
            <h1 className="text-5xl lg:text-6xl font-bold text-white mb-4 tracking-tight">{a.ai.title}</h1>
            <p className="text-lg text-slate-200 leading-relaxed">{a.ai.description}</p>
          </div>
        </section>
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-4 mb-8">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center"
                style={{ background: 'rgba(108,172,228,0.2)', border: '1px solid rgba(108,172,228,0.3)' }}>
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
              {a.ai.features.map((f, i) => (
                <li key={i} className="card-dark flex items-center gap-3 p-5 rounded-xl">
